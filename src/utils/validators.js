...
export const validateCatalogFields = (catalog) => {
    if (!catalog.name || catalog.name.trim().length === 0) {
        return { isValid: false, message: "El nombre del catálogo es obligatorio" };
    }
    if (!catalog.description || catalog.description.trim().length === 0) {
